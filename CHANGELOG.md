...
- Adiciona **`minimax-h3-prompter`** em `producao-ia/`, skill para o MiniMax H3 — o primeiro modelo do repositório que gera **vídeo e áudio juntos, nativamente**, em toda geração. Isso muda o desenho da skill: enquanto `seedance-prompter` exige `music: none` e joga a trilha pro `trilha-sonora-prompter`, aqui o som é dirigido em três camadas dentro do próprio prompt (diegético sincronizado no corpo, ambiente em `overall_soundscape`, trilha em `non_diegetic_music`), e trocar uma camada pela outra é erro de formato, não de gosto.
- A skill escreve no **formato nativo publicado pela MiniMax**, não em prosa livre: cinco modos (T2VA/I2VA/FL2VA/L2VA/Ref2VA), campos com nomes fixos, linhas de instrução de alinhamento literais, e notação `[Shot N]` / `(Sx)` / `<d>[Idioma] …</d>` / `<scenetrans>`. Inclui 9 reference files; `OFFICIAL_SYNTAX.md`, `BASE_MODES.md` e `REF2VA_MODE.md` foram extraídos dos dois guias oficiais (`VIDEO_PROMPT_WRITING_GUIDE_base_en.md` e `..._ref_en.md`), com os exemplos completos verbatim.
- Cobre o que o Seedance não faz: diálogo com lip-sync (inclusive falas atravessando cortes), clone de voz via `<Audio N>`, edição localizada de vídeo existente, continuação de vídeo e texto legível em tela. A tabela no fim do `SKILL.md` diz quando preferir cada um — H3 não substitui os dois `seedance-prompter`(s), divide trabalho com eles.
- `OFFICIAL_SKILLS.md` cataloga as 8 skills de estilo publicadas pela MiniMax, com a ressalva de que todas exigem o MiniMax Hub e **não rodam fora dele** — o que viaja é o playbook (style lock global, gates de aprovação antes de gerações caras, shot table antes de prompt), não o código.
