...
- **`diretor-cinematografico`** — when a shot needs actor direction (that skill's micro-beats reference file), the character's `MOVEMENT`/`STILLNESS` lock is the starting point for that character's baseline physicality; the director skill adds the scene-specific emotional beat on top of it.
